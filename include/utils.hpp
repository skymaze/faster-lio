...
class Timer {
   public:
    ...
    static void PrintAll() {
        RCLCPP_INFO(rclcpp::get_logger("rclcpp"), ">>> ===== Printing run time =====");
        for (const auto& r : records_) {
            RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "> [ %s ] average time usage: %f ms , called times: %ld",
                        r.first.c_str(),
        ...
        RCLCPP_INFO(rclcpp::get_logger("rclcpp"), ">>> ===== Printing run time end =====");
    }
    ...
    static void DumpIntoFile(const std::string& file_name) {
        ...
        if (!ofs.is_open()) {
            RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Failed to open file: %s", file_name.c_str());
            return;
        } else {
            RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Dump Time Records into file: %s", file_name.c_str());
        }
