...
void ImuProcess::SetGyrCov(const V3D &scaler) { cov_gyr_scale_ = scaler; }

void ImuProcess::SetAccCov(const V3D &scaler) { cov_acc_scale_ = scaler; }
...
void ImuProcess::Process(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state,
    ...
    if (imu_need_init_) {
        ...
        if (init_iter_num_ > MAX_INI_COUNT) {
            ...
            RCLCPP_INFO(rclcpp::get_logger("imu_preprocess"), "IMU Initial Done");
